...
library(RWeka)
library(readxl)
import_data <- read_excel("C:/Users/Lenovo/OneDrive - Drexel University/mis group 4 (project)/student_dataset.xlsx" ,  sheet = "Return and improvement - Emp")

#Dataset column to read the text
text_data <- import_data$`What are the student's weaknesses/areas for improvement?`
...
clean_corpus <- function(corpus){
  corpus   %>%
    ...
    tm_map( content_transformer(function(x) gsub(x, pattern = "technic", replacement = "technical" ))) %>%  #Remove a custom Stemming
    ...
    tm_map( content_transformer(function(x) gsub(x, pattern = "solv", replacement = "solve" ))) %>%  #Remove a custom Stemming
    tm_map( content_transformer(function(x) gsub(x, pattern = "posit", replacement = "positive" ))) %>%  #Remove a custom Stemming
    tm_map( content_transformer(function(x) gsub(x, pattern = "veri", replacement = "verify" ))) %>%  #Remove a custom Stemming
...
library("xlsx")
write.xlsx(frequency_terms, file = "std_freq.xlsx",
           sheetName = "std", append = FALSE)
